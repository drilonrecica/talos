# Binnacle v0.1.0-alpha.1 Pre-Release Audit
...
|---|----------|---------|----------------------|-----|
| 1 | Critical | Health check in `Dockerfile` and `docker-compose.yml` hit `GET /api/v1/session`, which requires a valid session and returns 401 on a fresh install. Container is marked unhealthy before first login. | SPEC §8.2, §44.1 | Added `--healthcheck` CLI flag that performs a one-shot HTTP GET to the existing unauthenticated `/healthz` endpoint. Updated Dockerfile and Compose health checks to use `binnacle --healthcheck`. |
| 2 | Critical | Docker Compose set `user: binnacle` but the `binnacle` user is not in the host `docker` group, so the Docker collector is denied socket access by default. | SPEC §8.2 | Changed Compose to `user: "binnacle:${DOCKER_GID}"` and documented the required `export DOCKER_GID=$(getent group docker \| cut -d: -f3)`. |
| 3 | Critical | Demo mode used `DemoAuthorizer`, which authorized every request. Accidental `BINNACLE_DEMO=true` in production granted full unauthenticated admin access. | SPEC §37, §46 | Refactored `cmd/binnacle/main.go` so auth, setup, onboarding, and settings services are initialized in demo mode too. Demo mode now only replaces real host/Docker collection with synthetic data; authentication is still required. |
...
| 6 | High | `events` inserts hard-coded `severity='info'` and `source='binnacle'` and omitted `host_id`, `details_json`, and `correlation_key`. | SPEC §23.10 | Updated `storage/writer.go` to derive severity from event type, set `host_id`, use a type-appropriate source, and populate `details_json` when available. |
...
| 11 | Medium | `make dev`, `make dev-demo`, and `make dev-host` were stubs that exited with error. | SPEC §44.1, TASKS T005 | Replaced stubs with working targets that build the binary and run it with a fresh temporary `BINNACLE_DATA_DIR` and a loopback listen address. `dev` also starts the Vite dev server. |
...
| 14 | Low | `settings.Defaults()` hard-coded `RuntimeDir` to `/var/lib/binnacle/runtime`, overriding `BINNACLE_DATA_DIR`. | SPEC §13.2 | Removed the default runtime dir and made `Normalize()` derive it from `DataDir` when unset, so `BINNACLE_DATA_DIR` affects all paths. |
...
| 20 | Medium | Go toolchain was pinned to an unreleased `1.26.4` local build. | SPEC §9.1, §44 | Pinned the project to Go `1.26` (latest public stable, patch `1.26.5`) in `go.mod`, CI workflows, and the Dockerfile. Updated all indirect Go dependencies and documented the inventory in `docs/DEPENDENCIES.md`. |
...
| 22 | Medium | Frontend dev dependencies were behind latest stable majors and Node/pnpm engine definitions did not match the Dockerfile. | SPEC §9.2, §44 | Bumped Node.js to 24.x LTS, pnpm to 11.12.0, Vite to 8, ESLint to 10, `@sveltejs/vite-plugin-svelte` to 7, and other dev deps to latest compatible versions. Documented the inventory and the TypeScript 7 exception in `docs/DEPENDENCIES.md`. |
| 23 | Low | GitHub Actions used older tags or floating refs (`trivy-action@master`, `sbom-action@v0`, `attest-build-provenance@v1`). | SPEC §44 | Pinned all actions to their latest stable semantic-version tags, including Pages, Docker, trivy, sbom, attest, and codeql actions. |
...
1. `./bin/binnacle --demo --demo-seed 42 --demo-containers 5` starts with `BINNACLE_DATA_DIR`, `BINNACLE_LISTEN_ADDRESS=127.0.0.1:8080`, and `BINNACLE_SETUP_TOKEN` set.
2. `./bin/binnacle --healthcheck` returns exit code 0 against the running instance.
3. `GET /api/v1/server` returns HTTP 401 when unauthenticated (correct behavior after the demo-auth fix).
4. `docker compose -f packaging/docker/docker-compose.yml config` validates syntax when `DOCKER_GID` and `BINNACLE_SETUP_TOKEN` are supplied.
